...
def draw_shape_maps():
    ...
    print("\n===== Drawing country shape images =====")

    gdf = gpd.read_file(COUNTRIES_GPKG, columns=["ISO_A3", "geometry", "NAM_0"],)

    output_dir = Path(COUNTRY_SHAPE_IMAGES_DIR)
    ...
    for i, (_, country) in enumerate(gdf.iterrows(), start=1):
        ...
        # Project all countries once into local CRS
        projected = working_gdf.to_crs(local_crs)
        target = projected.loc[[country.name]]
        ...
        # Filter countries after projection
        nearby = projected.cx[xmin:xmax, ymin:ymax]
